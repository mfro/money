...
export namespace Transaction {
  export function load(raw: string[]): Transaction {
    const date = Date.load(raw[0]);
    const description = raw[1];
    const value = { cents: Money.load(raw[3]).cents - Money.load(raw[2]).cents };
    const category = raw[4];
    const balance = Money.load(raw[5]);

    return { date, description, value, category, balance };
  }
